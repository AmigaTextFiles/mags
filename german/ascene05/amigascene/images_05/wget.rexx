 ...
 * Copyright (C) 1998 by Thomas Aglassinger <agi@sbox.tu-graz.ac.at>
 *
...
/* Version information */
version = "$VER: wget.rexx 2.1 (13.5.1998)"
...
/* Parse CLI arguments */
template = "Verbose/S,To/K,Ask/S,Further/S,Port/K,Continue/S,Clip/K,Options/F"
...
IF verbose THEN DO
    ...
    SAY 'Copyright © 1998 Thomas Aglassinger. Freeware.'
    SAY
END
...
IF RC ~= 0 THEN DO
    CALL view_error_message(RC_Error, "Could not find 'wget'.*NMake sure it is in the Workbench search path.")
END

/* Check, if download directory is available */
download_directory = to
IF ~Exists(download_directory) THEN DO
    CALL view_error_message(RC_Error, "Download directory '" || download_directory || "' not found.*NMake sure it exists and that it is accessible.")
END
...
IF ~SHOW('P', browser.port) THEN DO
    ...
    CALL view_error_message(RC_Error, "Could not find WWW browser in port '" || browser.port || "'.*NStart browser before using this script.")
END
...
IF (ask) THEN DO
    ...
    ok = 0
    ask_options = rtGetString(ask_options, "     Enter" || further_text || "options for Wget     ", "Wget.rexx Request",,,ok)
    IF (ok = 0) THEN DO
        EXIT 5
    END
...
IF (POS('http://', options) > 0) THEN DO
    CALL view_error_message(RC_Error, "Only one URI at a time allowed in options.*NRemove all other URIs from your request.")
END
...
IF server = '' THEN DO
    ...
    CALL view_error_message(RC_Error, " Server name is missing in URI*Nor does not start with 'http://'")
END
...
wget_command = 'wget -x --directory-prefix=' || target_directory
...
NoValue:
    ...
    SAY 'wget.rexx: error in line ' || SIGL || ': uninitialized value.'
    EXIT 20
